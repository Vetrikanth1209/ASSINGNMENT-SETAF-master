...
function Selva() {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await axios.post('http://localhost:1234/student', formData);
  ...
  const validateForm = (data) => {
    ...
    if (!data.certificate_pdf) {
        errors.certificate_pdf = '* Certificate PDF is required';
    }
              ...
              <div className="shadow-lg rounded-4 p-3" style={{ marginTop: '30px', marginBottom: '30px', backgroundColor: 'white' }}>
                ...
                <div>
                  <Typography variant="h5" color="#32348c" align="center" sx={{ marginTop: 2 }}>
                    <b>STUDENT FIELD WORK AND INTERSHIP GUIDANCE</b>
                  </Typography>
                  <TextField label="Name of the Faculty" fullWidth sx={{ marginTop: 2 }} variant="outlined" name='name_of_the_faculty'
                    value={formData.name_of_the_faculty}
                    onChange={handleChange} 
                    error={!!errors.name_of_the_faculty}
                    helperText={errors.name_of_the_faculty}/>
                    <TextField label="Designation" fullWidth sx={{ marginTop: 2 }} variant="outlined" name='designation'
                  ...
                  <TextField label="Title Of The Program" fullWidth sx={{ marginTop: 2 }} variant="outlined" name='title_of_the_program'
                    value={formData.title_of_the_program}
                    onChange={handleChange} 
                    error={!!errors.title_of_the_program}
                    helperText={errors.title_of_the_program}/>
                    <TextField label="Start Date" fullWidth sx={{ marginTop: 2 }} variant="outlined" name='date_from'
                    value={formData.date_from}
                    onChange={handleChange} 
                    error={!!errors.date_from}
                    helperText={errors.date_from}/>
                     <TextField label="End Date" fullWidth sx={{ marginTop: 2 }} variant="outlined" name='date_to'
                  ...
                  <TextField label="Location Of Organization" fullWidth sx={{ marginTop: 2 }} variant="outlined" name='location_of_organization'
                    value={formData.location_of_organization}
                    onChange={handleChange}
                    error={!!errors.location_of_organization}
                    helperText={errors.location_of_organization}/>
                  <TextField label="Amount Provided by HEI" fullWidth sx={{ marginTop: 2 }} variant="outlined" name='amount_provided_by_the_hei'
